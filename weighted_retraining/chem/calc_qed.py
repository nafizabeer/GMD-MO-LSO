""" calculate QED for all smiles in a file """
...
parser.add_argument(
    ...
    type=str,
    nargs="+",
    help="list of file of SMILES to calculate QED for",
    required=True,
)
parser.add_argument(
    "-o",
...
if __name__ == "__main__":
    ...
    prop_dict = dict()
    for smiles in tqdm(input_smiles, desc="calc QED", dynamic_ncols=True):
        c_smiles = standardize_smiles(smiles)
        qed = qed_func(c_smiles)
        prop_dict[smiles] = qed
        prop_dict[c_smiles] = qed
